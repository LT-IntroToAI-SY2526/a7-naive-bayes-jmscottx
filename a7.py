...
class BayesClassifier:
    ...
    def train(self) -> None:
        ...
        for index, filename in enumerate(files, 1):
            ...
            # Check prefix and update the correct dictionary
    ...
    def classify(self, text: str) -> str:
        """Classifies text using log-likelihood and add-one smoothing"""
        tokens = self.tokenize(text)
        # Calculate total word counts (denominators)
        sum_pos = sum(self.pos_freqs.values())
        sum_neg = sum(self.neg_freqs.values())
        # Calculate unique words (vocabulary size) for Laplace smoothing
    ...
    def tokenize(self, text: str) -> List[str]:
        ...
        if token != "":
            tokens.append(token.lower())
        return tokens
    def update_dict(self, tokens: List[str], freq_dict: Dict[str, int]) -> None:
        """Updates the specific dictionary with tokens from a file"""
        for token in tokens:
